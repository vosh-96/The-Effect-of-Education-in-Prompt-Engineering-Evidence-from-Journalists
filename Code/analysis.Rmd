...
expert_evaluated_raw_data_name <- "annotated_data_240822.csv"
expert_evaluated_raw_df <- read_csv(file.path(getwd(),"..", "Data", expert_evaluated_raw_data_name), 
...
layperson_evaluated_raw_df  <- read_csv(file.path(getwd(),"..", "Data", layperson_evaluated_raw_data_name), 
                   show_col_types = FALSE)


question_mapping_data_name <- "Question_name_mapping.csv"
question_mapping_df  <- read_csv(file.path(getwd(),"..", "Data", question_mapping_data_name), 
                   show_col_types = FALSE)
...
#pdf("perceived_expertise.pdf",width = 6, height = 4)


boxplot_perceived_expertise <- ggwithinstats(
  data = expert_evaluated_completed_tasks_df,
  x = training_status, 
  y = perceived_expertise,
...
```{r}
pdf("perceived_expertise.pdf",width = 4, height = 4)
...
# pdf("percieved_helpfulness.pdf",width = 6, height = 4)
boxplot_expert_helpfulness <- ggwithinstats(
  data = expert_evaluated_completed_tasks_df,
  x = training_status, 
  y = perceived_helpfulness,
  pairwise.display = "all",
  centrality.plotting = FALSE,
  xlab = element_blank(),
  ylab = "Participants' Perceived Helpfulness with LLM",
  centrality.point.args = list(size = 3.5, color = c("#FF9800", "#1976D2")),
  point.path = FALSE,
...
```{r}
pdf("perceived_helpfulness.pdf",width = 4, height = 4)
...
```
```{r}
boxplot_expert_score_misrep <-  ggbetweenstats(
  data = expert_evaluated_completed_tasks_df,
  x = article_training_mutation,
  y = expert_grade_representation,
   pairwise.display = "all",
  centrality.path = FALSE,
  xlab = "",
  ylab = "representation errors in posts",
  centrality.plotting = FALSE,
  point.path = FALSE,
  results.subtitle = TRUE,
#  type = "robust",
...
boxplot_expert_score_misrep + scale_y_continuous(limits = c(0, 4))
```
...
```{r}
metrics_expert_list <- c("expert_grade_factual", "expert_grade_representation", "expert_grade_reward")
...
```{r}
pdf("RQ3_layperson.pdf",width = 6, height = 6)
...
```
```{r}
pdf("expert_score_overall.pdf",width = 10, height = 4)
ggplot(result , 
       aes(x = metrics, y = mean_val, 
           fill = article_training_mutation, 
           pattern = article_training_mutation,
...
metrics_list <- c("Readability", "Clarity", "Informativeness", "Engagement", 
...
pdf("RQ3_layperson.pdf",width = 18, height = 10)
...
```{r}
between_similarity_df_a <- readxl::read_excel('../Data/between_subject_similarity_matrix_a.xlsx')
between_similarity_matrix_a <- data.matrix(between_similarity_df_a)
between_similarity_df_b <- readxl::read_excel('../Data/between_subject_similarity_matrix_b.xlsx')
